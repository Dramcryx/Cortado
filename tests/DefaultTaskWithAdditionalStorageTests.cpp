...
struct UserStorage
{
    ...
    inline static std::unordered_map<std::string,
        std::pair<unsigned long, unsigned long>> TestResultStorage;
    ...
    ~UserStorage()
    {
        ...
        TestResultStorage[m_testName] = {
            BeforeSuspendCallCount.load(),
...
private:
    std::string m_testName = ::testing::UnitTest::GetInstance()->current_test_info()->name();
};
...
TEST(DefaultTaskWithAdditionalStorageTests, BasicTest)
{
    ...
    auto [beforeSuspend, _] =\
        UserStorage::TestResultStorage[::testing::UnitTest::GetInstance()->current_test_info()->name()];
...
TEST(DefaultTaskWithAdditionalStorageTests, ResumeBackgroundTest)
{
    ...
    for (;;)
    {
        ...
        std::tie(beforeSuspend, beforeResume) = UserStorage::TestResultStorage[testName];
        break;
    }
    EXPECT_EQ(2, beforeSuspend) << "Expected two calls";

    EXPECT_EQ(1, beforeResume) << "Expected one call in resume background awaiter";
}
