...
public:
	...
	TestAllocator(std::shared_ptr<SharedAllocatorState> state)
		: State(state)
	{
    }
...
{
	using Task = Cortado::Task<void, TaskImplWithCustomAllocator<TestAllocator>>;
...
TEST(CoroutineAllocatorTest, BasicAllocationDeallocationWithNonMovableAllocator)
{
	...
	using Task = Cortado::Task<void, TaskImplWithCustomAllocator<NonMovableAllocator>>;
    ...
    EXPECT_EQ(0, state->MoveConstructedCount)
        << "Moving must not be possible";
...
TEST(CoroutineAllocatorTest, AllocationFailureBehaivor)
{
	...
	using Task = Cortado::Task<void, TaskImplWithCustomAllocator<ThrowingAllocator>>;
